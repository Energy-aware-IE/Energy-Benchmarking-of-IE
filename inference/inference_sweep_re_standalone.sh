#!/bin/bash
# =============================================================================
# Sweep RE Inference Task (DocRED Relation Extraction)
# =============================================================================
# Runs RE evaluation (re_lang_eval_mlflow_mi_gol_all9_semaphore.py) across all
# requested prompt styles for a sweep run.
#
# Called by sweep_single_run_slurm_re.sh (via srun bash), NOT submitted directly.
#
# Required env vars (all exported by sweep_single_run_slurm_re.sh):
#   SWEEP_SPLIT, SWEEP_PROMPT_STYLES, SWEEP_BATCH_SIZE,
...
# =============================================================================
echo "[INFO] === RE Sweep Inference Task ==="
...
# Bridge sweep infrastructure name → what the eval Python script reads.
# The eval script reads MLFLOW_EXPERIMENT_NAME (falls back to prompts_en_/... default).
# SWEEP_MLFLOW_EXPERIMENT is set by sweep_single_run_slurm_re.sh via SWEEP_ENV_FILE.
...
OUT_INFERENCE_CONFIG="${ARTIFACTS_DIR}/sweep_${SWEEP_PARAM_CHANGED}_job_${SLURM_JOB_ID}_m_${MODEL_NAME}_re_out_inference.json"
...
for style in "${prompt_styles[@]}"; do
    ...
    # All SWEEP_* and MLFLOW_* env vars are already exported by sweep_single_run_slurm_re.sh.
    python3 "${EVAL_PY}" \
            --split "${split}" \
            --model "/${MODEL_NAME}" \
