#!/bin/bash
# =============================================================================
# Sweep Orchestrator — EE (RAMS Event Argument Extraction)
# =============================================================================
# Submits one SLURM job per parameter variation (ablation study) for the EE task.
# Design: one parameter changes at a time; all others stay at baseline.
#
# Usage:
#   bash sweep_orchestrator_ee.sh [--dry-run]
#
# Output:
#   - One SLURM job per (model, param, value) combination
#   - Job registry CSV: sweep_registry/ee_job_registry_<timestamp>.csv
#   - Per-run env files: sweep_configs/ee_run_<id>.env  (sourced by SLURM job)
#
# To monitor after submission:
#   squeue -u $USER
#   column -t -s, sweep_registry/ee_job_registry_<timestamp>.csv | less -S
#
...
#
# NOTE: Supported EE models (as of ee_lang_eval_mlflow_mi_gol_all9_semaphore.py):
...
SWEEP_SLURM_SCRIPT="${HOST_BASE}/sweep_single_run_slurm_ee.sh"
SWEEP_CONFIGS_DIR="${HOST_BASE}/sweep_configs"
REGISTRY_DIR="${HOST_BASE}/sweep_registry"
RESULTS_BASE="${HOST_BASE}/experiment_sweep_results/EE"

mkdir -p "${SWEEP_CONFIGS_DIR}" "${REGISTRY_DIR}" "${RESULTS_BASE}"
mkdir -p "${HOST_BASE}/slurm_out/slurm_sweep_ee"

# RAMS dataset path (contains data/dev.jsonlines, data/test.jsonlines, etc.)
RAMS_DIR="${REPO_ROOT}/data/datasets/RAMS_1.0c"
...
declare -A PARAM_VARIATIONS=(
    ...
    [max_new_tokens]="100 150 200 250 300"
...
write_env_file() {
    ...
    cat > "${env_file}" <<EOF
# Auto-generated by sweep_orchestrator_ee.sh — do not edit manually.
# (model=${model}, task=ee, param_changed=${override_param}, val=${override_val})

SWEEP_MODEL="${model}"
SWEEP_TASK="ee"
SWEEP_SPLIT="dev"
SWEEP_RAMS_DIR="${RAMS_DIR}"
...
submit_job() {
    ...
    sbatch \
        --job-name="${job_name}" \
        --output="${HOST_BASE}/slurm_out/slurm_sweep_ee/slurm_%x_%j.out" \
        --error="${HOST_BASE}/slurm_out/slurm_sweep_ee/slurm_%x_%j.err" \
...
REGISTRY_FILE="${REGISTRY_DIR}/ee_job_registry_${TIMESTAMP}.csv"
...
echo "[INFO] EE Sweep started: ${TIMESTAMP}"
...
for model in "${MODELS[@]}"; do
    MLFLOW_EXP="sweep_ee_${model}_${TIMESTAMP}"
    ARTIFACTS_BASE="${RESULTS_BASE}/sweep_ee_${model}_${TIMESTAMP}"
    mkdir -p "${ARTIFACTS_BASE}"

    echo "[INFO] ==========================================================="
    echo "[INFO] EE Sweep: model=${model}  task=ee  split=dev"
    echo "[INFO] MLflow experiment: ${MLFLOW_EXP}"
    echo "[INFO] Artifacts base: ${ARTIFACTS_BASE}"
    echo "[INFO] ==========================================================="

    # ── 1. Baseline run ──────────────────────────────────────────────────────
    BL_ENV="${SWEEP_CONFIGS_DIR}/ee_run_${model}_BASELINE_${TIMESTAMP}.env"
    write_env_file "${BL_ENV}" "${model}" \
        "BASELINE" "BASELINE" "${MLFLOW_EXP}" "${ARTIFACTS_BASE}"

    JOB_NAME="sw_ee_bl_${model:0:8}"
    JOB_ID=$(submit_job "${BL_ENV}" "${JOB_NAME}")
    SUBMITTED_AT=$(date +"%Y-%m-%dT%H:%M:%S")
    echo "[INFO] Baseline job submitted: ${JOB_ID}"

    printf '%s\n' \
        "${JOB_ID},${SUBMITTED_AT},${model},ee,dev,BASELINE,BASELINE,\"${BASELINE[prompt_styles]}\",${BASELINE[max_concurrency]},${BASELINE[batch_size]},${BASELINE[max_new_tokens]},${BASELINE[max_model_len]},${BASELINE[max_num_seqs]},${BASELINE[max_num_batched_tokens]},${BASELINE[kv_cache_dtype]},${BASELINE[block_size]},${BASELINE[dtype]},${BASELINE[enable_prefix_caching]},${BASELINE[enable_chunked_prefill]},${BASELINE[calculate_kv_scales]},${BASELINE[tensor_parallel_size]},${BASELINE[gpu_memory_util]},${MLFLOW_EXP},${ARTIFACTS_BASE},${BL_ENV}" \
        >> "${REGISTRY_FILE}"
...
echo "[INFO] All EE jobs submitted."
echo "[INFO] Registry: ${REGISTRY_FILE}"
echo ""
