#!/bin/bash
# =============================================================================
# Sweep Single Run — SLURM Job Script (EE / RAMS)
# =============================================================================
# One SLURM job = one parameter configuration for EE sweep.
# All parameters are read from the env file pointed to by SWEEP_ENV_FILE.
# Do NOT submit this script directly — use sweep_orchestrator_ee.sh.
#
# Required: SWEEP_ENV_FILE must be set (done by sweep_orchestrator_ee.sh via
...
#SBATCH --output=/path/to/repo/src/serve_vllm/slurm_out/slurm_sweep_ee/slurm_%x_%j.out
#SBATCH --error=/path/to/repo/src/serve_vllm/slurm_out/slurm_sweep_ee/slurm_%x_%j.err
...
if [[ -z "${SWEEP_ENV_FILE:-}" ]]; then
    echo "[FATAL] SWEEP_ENV_FILE is not set. Submit via sweep_orchestrator_ee.sh."
    exit 1
fi
...
export HOST_MODEL
export EXPERIMENT_RESULT_BASE="${HOST_BASE}/experiment_sweep_results/EE"
...
# EE-specific: RAMS directory and split
export SWEEP_SPLIT="${SWEEP_SPLIT:-dev}"
export SWEEP_RAMS_DIR="${SWEEP_RAMS_DIR}"
...
# Inference task: always EE standalone (format dispatch is inside the script)
INFERENCE_TASK="${HOST_BASE}/inference_sweep_ee_standalone.sh"
...
# =============================================================================
echo "======================================================================"
echo "  EE Sweep Run Summary"
echo "  Job ID          : ${SLURM_JOB_ID}"
echo "  Node            : $(hostname)"
echo "  Model           : ${SWEEP_MODEL}"
echo "  Task            : EE (RAMS)"
echo "  Split           : ${SWEEP_SPLIT}"
echo "  RAMS dir        : ${SWEEP_RAMS_DIR}"
...
SERVCONF_PATH="${EXPERIMENT_CONFIGS_DIR}/sweep_${SWEEP_PARAM_CHANGED}_job_${SLURM_JOB_ID}_m_${SWEEP_MODEL}_ee_out_servconf.json"
...
if [[ "${DCGM_OK}" == "false" ]]; then
    ...
    sbatch \
        ...
        -o "${HOST_BASE}/slurm_out/slurm_sweep_ee/slurm_sw_ee_retry_${SLURM_JOB_ID}_%j.out" \
        -e "${HOST_BASE}/slurm_out/slurm_sweep_ee/slurm_sw_ee_retry_${SLURM_JOB_ID}_%j.err" \
        "${BASH_SOURCE[0]}"
